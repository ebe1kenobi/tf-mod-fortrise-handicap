...
{
  public class MyTeamDeathmatchRoundLogic : IHookable
  {
    public static void Load(IHarmony harmony)
    {
      harmony.Patch(
          AccessTools.DeclaredMethod(typeof(TeamDeathmatchRoundLogic), nameof(TeamDeathmatchRoundLogic.OnPlayerDeath)),
          prefix: new HarmonyMethod(OnPlayerDeath_patch)
      );
    }

    public static bool OnPlayerDeath_patch(TeamDeathmatchRoundLogic __instance, Player player, PlayerCorpse corpse, int playerIndex, DeathCause deathType, Vector2 position, int killerIndex)
    {
